...
def test(model, device, test_loader):
    ...
    test_loss = 0
    test_ccc = 0
    test_loss_list = []  # Changed from test_loss = 0
    test_ccc_list = [] 
    y_hat_all = []
    y_all = []
    cell_names = []  
    with torch.no_grad():
        for batch_idx, (x, y) in enumerate(test_loader):
            ...
            # Calculate which cell indices this batch represents
            batch_size = len(x)
            for i in range(batch_size):
              dataset_idx = batch_idx * test_loader.batch_size + i
              if dataset_idx < len(test_loader.dataset):
                  cell_name = test_loader.dataset.scRNA_adata.obs.index[dataset_idx]
            ...
            # test_loss_list.append(F.mse_loss(y_hat[pro_mask], y[pro_mask], reduction='none').cpu().numpy())
            # test_ccc_list.append(loss2(y_hat[pro_mask], y[pro_mask]).cpu().numpy())
    ...
    y_hat_array = np.array(y_hat_all)
    y_array = np.array(y_all)
    
    if len(y_hat_array.shape) == 1:
        # If 1D, treat as single feature
        from scipy.stats import pearsonr
        test_pcor = pearsonr(y_hat_array, y_array)[0]
    else:
        # If 2D, compute correlation for each feature (column)
        test_pcor = []
        from scipy.stats import pearsonr
        for feature_idx in range(y_hat_array.shape[1]):
            try:
                corr, _ = pearsonr(y_hat_array[:, feature_idx], y_array[:, feature_idx])
                test_pcor.append(corr if not np.isnan(corr) else 0.0)
            except:
                test_pcor.append(0.0)
        test_pcor = np.array(test_pcor)


    test_loss /= len(test_loader)
    test_ccc /= len(test_loader)
    return test_loss, test_ccc, test_pcor, np.array(y_hat_all), np.array(y_all), cell_names
