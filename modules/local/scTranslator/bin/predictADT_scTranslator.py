...
num_train_samples = train_rna.n_obs  # Number of cells, not features
...
if dobenchmark=='true':
    ...
    log_path = prefix + str(rank)+  '_' + str(args.ncells) + 'cells_finetune_rank_log.csv'
    ...
    test_pcor.to_csv(prefix +  '_' + str(args.ncells) + 'cells_finetune_pearson_corr.csv')
    # test_ccc.to_csv(prefix + '_finetune_ccc.csv')
    # test_loss.to_csv(prefix + '_finetune_loss.csv')
    
    y_pred.to_csv( prefix +  '_' + str(args.ncells) + "cells_scTranslator_finetune_prediction.csv")
    y_truth.to_csv( prefix + '_' + str(args.ncells) +  "cells_scTranslator_finetune_truth.csv")
    ...
    log_path =  str(rank)+ '_' + str(args.ncells) +'cells_finetune_rank_log.csv'
    log_all = pd.DataFrame(columns=['train_loss', 'train_ccc', 'test_loss', 'test_ccc'])
    log_all.loc[args.repeat] = np.array([train_loss, train_ccc, test_loss, test_ccc])
    log_all.to_csv(log_path)
    ...
    test_pcor.to_csv(str(args.ncells)  + 'cells_finetune_pearson_corr.csv')
    # test_ccc.to_csv( 'fewshot_ccc.csv')
    # test_loss.to_csv( 'fewshot_loss.csv')
    
    #pass downstream to pipe
    y_pred.to_csv( str(args.ncells)  + "cells_scTranslator_finetune_prediction.csv")
    y_truth.to_csv(  str(args.ncells) + "cells_scTranslator_finetune_truth.csv")
