...
#set colors
method_cols <- c(BABEL  ="#F79BB2", sciPENN="#e6173a", cTP_net= "#EB7926FF", totalVI= "#FFBB44FF", scMMT= "#859B6CFF", SPECK= "#4FD3DA", scLinear= "#006B62", Seurat = "black", scTranslator_pretrain = '#b18dc4', scTranslator_fewshot =  '#4e1c69')
...
for(i in c(1,0.5,0.25,0.125,0.0625)){
  ...
  benchmark_predset[[as.character(i)]][['scor']] <- scorrList
  benchmark_predset[[as.character(i)]][['RMSE']] <-RMSEList
  benchmark_predset[[as.character(i)]][['NRMSE']] <-NRMSEList
  benchmark_predset[[as.character(i)]][['pred']] <-predList
  benchmark_predset[[as.character(i)]][['truth']] <-truthList
}
...
for(i in c('_B','_CD4 T' , '_CD8 T', '_DC','_Mono','_NK')){
  ...
  leave_out_predset[[as.character(i)]][['scor']] <- scorrList
  leave_out_predset[[as.character(i)]][['RMSE']] <-RMSEList
  leave_out_predset[[as.character(i)]][['NRMSE']] <-NRMSEList
  leave_out_predset[[as.character(i)]][['pred']] <-predList
  leave_out_predset[[as.character(i)]][['truth']] <-truthList
}
...
for(i in c('B','CD4 T' , 'CD8 T', 'DC','Mono','NK')){
  ...
  one_only_predset[[as.character(i)]][['scor']] <- scorrList
  one_only_predset[[as.character(i)]][['RMSE']] <-RMSEList
  one_only_predset[[as.character(i)]][['NRMSE']] <-NRMSEList
  one_only_predset[[as.character(i)]][['pred']] <-predList
  one_only_predset[[as.character(i)]][['truth']] <-truthList
}
...
scale_color_manual(values = sample(alpha(colorRampPalette(cols)(length(unique(obj$celltype[order(obj$general_celltype)]))),0.2)) )+
  ggtitle('')+
...
protdf <- FetchData(testobj, ref$protein[ref$dataset=='Hao_et_al'])
DefaultAssay(testobj) <-'RNA'
genedf <- FetchData(testobj, ref$gene[ref$dataset=='Hao_et_al'])
genedf <- genedf[,ref$gene[ref$dataset=='Hao_et_al']]
...
```{r}
ref <- ref[ref$dataset=='Hao_et_al',]
#plot individual prot vs RNA examples
feat <- ref$protein[ref$dataset=='Hao_et_al']
plotList <- list()
corVals <- list()
for(f in feat){
...
pseudo <- AggregateExpression<- dat(testobj, assays = "RNA", return.seurat = T, group.by = c("donor","celltype"))
...
cordf <- cordf %>% complete(feature, method, fill = list(value = NA))
...
cordf$method <- factor(cordf$method, levels = c( "Seurat" ,"BABEL","sciPENN","cTP_net","totalVI","scMMT","scLinear","scTranslator_fewshot","scTranslator_pretrain", "SPECK"  ) )
cordf$group <- factor(cordf$group, levels =c('(0.773,114]' ,'(114,228])' ))
p <- ggplot(cordf, aes(x = method, y = feature, fill = value))+
  ...
  scale_fill_gradientn(colors=rev(geyser_adjust),limits = c(-1,1), name='cor.', na.value = '#c9c9c9')+
...
ggsave(p,  file=paste0(outdir,'/1_1_prediction_heatmap.png'), width=8, height=18)
...
RMSEList <- benchmark_predset[['1']]$RMSE
...
#now plot RMSE
NRMSEList <- benchmark_predset[['1']]$RMSE
NRMSEdf <- lapply(names(NRMSEList), FUN=function(n){
...
NRMSEdf <- Reduce(rbind, NRMSEdf)
colnames(NRMSEdf) <- c('feature','value','method')
...
for(i in 1:dim(protdf)[2]){
  ...
  errorval <- errorval / sd(protvals)
  
  rna_NRMSE  <- c(rna_NRMSE, errorval)
  rna_cor <-c(rna_cor, cor(protvals, genevals))
}
names(rna_RMSE) <- colnames(protdf)
names(rna_NRMSE) <- colnames(protdf)
names(rna_cor) <- colnames(protdf)
rna_RMSE <- data.frame(feature = names(rna_NRMSE), value=rna_NRMSE, method='gene mRNA')
rna_NRMSE <- data.frame(feature = names(rna_NRMSE), value=rna_NRMSE, method='gene mRNA')
rna_cor <- data.frame(feature = names(rna_cor), value=rna_cor, method='gene mRNA')

RMSEdf <- rbind(RMSEdf, rna_RMSE)
NRMSEdf <- rbind(NRMSEdf, rna_NRMSE)
cordf <- rbind(cordf, rna_cor)
...
NRMSEdf$method <- factor(NRMSEdf$method, levels = c(rev(names(method_cols)), 'gene mRNA'))
#RMSEdf$value <- log(RMSEdf$value)
p3 <- ggplot(NRMSEdf, aes(y=method,x=value, fill=method))+
 geom_density_ridges(quantile_lines = TRUE, quantiles = 2)+
  theme_minimal()+
  scale_fill_manual(values=c('gene mRNA' ='#b3b3b3', method_cols), guide='none')+
ggtitle('NRMSE')+
...
agg <- cordf %>% group_by(feature) %>% dplyr::summarize(meancor = mean(na.omit(value)), maxcor = max(na.omit(value)))
agg <- agg[order(agg$maxcor, decreasing = T),]
agg <- as.data.frame(agg)
rownames(agg) <- agg$feature
cor_pred <- agg

training_data <- read.csv(paste0(savedir,"benchmarking/training_files/1_1_training_data_prot_norm.csv"), row.names=1)
...
#get total counts
training_data_raw <- read.csv(paste0(savedir,"benchmarking/training_files/1_1_training_data_prot_raw.csv"), row.names=1)
...
p1 <- ggplot(agg, aes(maxcor, counts))+
...
p2 <- ggplot(agg, aes(maxcor, sdvals))+
...
cor_metrics <- data.frame(expr = cor_expr, pos = cor_pos[names(cor_expr)], pseudo = cor_pseudo[names(cor_expr)], pred = cor_pred$maxcor[match(names(cor_expr), cor_pred$feature)])

#save file
tab <- cor_metrics
colnames(tab) <- c('mRNA norm. expression','%positivity','pseudobulk','ML prediction')
write.csv(tab, file = paste0(outdir,'/predcor_table.csv'))

cor_metrics[is.na(cor_metrics)] <- 0

#of the ones that fail RNA based methods, how many are rescued by ML prediction
ind <-which(rowSums(cor_metrics[,c('expr','pos','pseudo')] < 0.4) == 3)
...
## SUPP FIG 3 Spearman Correlation


```{r}
scorrList <- benchmark_predset[['1']]$scor

cordf <- lapply(names(scorrList), FUN=function(n){
...
ggsave(p,  file=paste0(outdir,'/1_1_spearman_prediction_heatmap.png'), width=8, height=18)
...
for(i in 1:dim(protdf)[2]){
  ...
  protvals <- protdf[,i]
  genevals <- genedf[,i]
  rna_cor <-c(rna_cor, cor(protvals, genevals, method='spearman'))
}
...
ggsave(p,  file=paste0(outdir,'/spearman_prediction_performance_density.png'), width=4.5, height=3.4)
...
p <- plot_grid(p1,p2, ncol=1)
ggsave(p,  file=paste0(outdir,'/spearman_prediction_dependence_on_signal.png'), width=2.5, height=5)
...
scale_color_manual(values = paletteer::paletteer_d("rcartocolor::Geyser")[c(7)], na.value =  as.character(paletteer::paletteer_d("rcartocolor::Geyser")[1]))+
  ggtitle('')+
  theme(legend.position='none',
...
df<- data.frame(expr = cor_expr, pred = cor_pred$maxcor[match(names(cor_expr), cor_pred$feature)])
...
for(n in names(benchmark_predset[['1']][['pred']])){
  ...
  pred <- benchmark_predset[['1']][['pred']][[n]][rownames(benchmark_predset[['1']][['pred']][[n]]) %in% feat, ,drop=F]
  colnames(pred) <- colnames(testobj)
  truth <- benchmark_predset[['1']]$truth[[n]][rownames(benchmark_predset[['1']][['truth']][[n]]) %in% feat, ,drop=F]
  
  for(ct in categories){

    cells <- colnames(testobj)[which(testobj$general_celltype==ct)]
    predmat <- pred[,cells]
    truthmat <- truth[,cells]
    protmat <- testobj[['protein']]@data[ref$protein,cells] %>% as.matrix()
    rnamat <- testobj[['RNA']]@data[ref$gene,cells] %>% as.matrix()
    
    cvals1 <- cor(t(predmat), t(truthmat)) %>% diag()
    ...
    df <- data.frame(pred_prot=cvals1[intersect(names(cvals1), names(cvals2))],rna_prot=cvals2[intersect(names(cvals1), names(cvals2))], celltype=ct, method = n ,medexpr = medvals[intersect(names(cvals1), names(cvals2))], feature = intersect(names(cvals1), names(cvals2)))
...
agg <- df %>% group_by(celltype, feature) %>% dplyr::summarize(meanpred = mean(pred_prot), maxpred = max(pred_prot),rna_prot = mean(rna_prot), medexpr = mean(medexpr))
agg$label <- NA
agg$celltype <- factor(agg$celltype, levels = c('NK','CD4 T','CD8 T','B','Mono','DC'))

p <- ggplot(agg, aes(x = rna_prot, y= maxpred, label=label, color=medexpr))+
...
preds <- lapply( 'Seurat', FUN=function(n){
  print(n)
...
for(feat in feats){
 ...
 # geom_point(size=0.1, alpha=0.1)+
  ...
  ylab('predicted expression (Seurat)')+
...
## FIG 3C Line plots of variations
...
#drop scTranslator
agg <- agg[-grep('scTranslator',agg$method),]
...
truthset <- benchmark_predset[['1']]$truth
# testing_data <- as.data.frame(read.csv(paste0(savedir,"benchmarking/testing_files/1_1_testing_data_prot_norm.csv"), row.names=1))
# testing_data_raw <- as.data.frame(read.csv(paste0(savedir,"benchmarking/testing_files/1_1_testing_data_prot_raw.csv"), row.names=1))
...
for(i in names(predset)){
  ...
  if(is.null(colnames(pred))){
      colnames(pred) <- colnames(truth)
  }
  print(i)
  
  dfs <- lapply(cts_cat, FUN=function(cat){
    print(cat)
    subpred <- pred[,rownames(meta)[which(meta$general_celltype == cat)]]
    subreal <- truth[,rownames(meta)[which(meta$general_celltype == cat)]]
...
#drop scTranslaotr
all_df <- all_df[-grep('scTranslator',all_df$method),]
...
p4 <- ggplot(loo_df, aes(y=value, color=method, x=partition, group=method))+
  ...
  theme(legend.text = element_text(size=10),
        ...
        axis.text.y = element_text(size=12),
        plot.title = element_text(size=15),
        plot.margin = margin(0.194, 1, 0.194, 0.194, "cm"))+
  ggtitle('Held-out Cell Type')




p <- plot_grid(p1,p2,p3,p4, ncol=4, rel_widths = c(0.2,0.21,0.21,0.34))
...
```{r}
#fetch files
file <- list.files(paste0(savedir,'/benchmarking/method_predictions/') ,pattern = '1_1_model_feature_importance', full.names = T)
imp <- read.csv(file, header = F)
#set rownames and colnames
rownames(imp) <- rownames(benchmark_predset[['1']]$pred$scLinear)
colnames(imp) <- rownames(testobj[['RNA']])
...
cor_longdf <- pivot_longer(cordf,cols =colnames(cordf)[1:10], names_to='method')
...
traces <- read.csv(paste0(savedir,'dataset_comparison_repeat/dataset_comparison_resource_and_runtime_metrics.csv'))
...
#group by process, rank memory usage and duration

agg <- traces %>% group_by(name) %>% summarise(meanmem = mean(peak_vmem_bytes), meandur = mean(realtime))
agg <- agg[agg$name %nin% c('sciPENN_init_conda','scTranslator_build_h5ad','eval_predictions', 'cTPnet_predict','Renv_init_conda',"preprocess_data","scTranslator_notrain"),]

agg$mem_score <- rank(-agg$meanmem)/dim(agg)[1]
agg$dur_score <- rank(-agg$meandur)/dim(agg)[1]

```



## SUPP FIG 4 Grid of dataset-dataset comparisons
...
cor_longdf$test <- factor(cor_longdf$test, levels = c('Hao_etal','Stephenson_etal_Covid','Stephenson_etal_Healthy','Leader_etal', 'Zhang_etal') )
cor_longdf$train <- factor(cor_longdf$train, levels =  c('Hao_etal','Stephenson_etal_Covid','Stephenson_etal_Healthy','Leader_etal', 'Zhang_etal') %>% rev() )

cor_longdf$method <- factor(cor_longdf$method, levels = names(method_cols))

#heatmap or corr, worted by tissue
p1 <- ggplot(cor_longdf, aes(x =test ,y = train, fill = value, label = round(value,1) ))+
  geom_tile(color='black')+
  geom_text(size=3)+
  ...
  theme_minimal()+
  theme(axis.text.x = element_text(color = tissue_cols[cor_longdf$test_tissue[match(levels(cor_longdf$test), cor_longdf$test)]], angle=60, vjust=1, hjust=1), 
        axis.text.y = element_text(color = tissue_cols[cor_longdf$train_tissue[match(levels(cor_longdf$train), cor_longdf$train)]]))+
...
## SUPP TAB 3 Dataset-to-Dataset Values
...
for(n in unique(names(datacomp_list))){
  set <- datacomp_list[[n]][['correlation']]
  vals <- lapply(names(set), FUN=function(i){
    x <- set[[i]]
    data.frame(features = names(x), values = x, n)
    })
  corrSets[[n]] <- Reduce(rbind, vals)
}

cordf <- Reduce(rbind, corrSets)
cordf$train <- lapply(cordf$n, FUN=function(x){str_split(x,'_vs_')[[1]][1]}) %>% unlist()
cordf$test <- lapply(cordf$n, FUN=function(x){str_split(x,'_vs_')[[1]][2]}) %>% unlist()
cordf$comparison <- cordf$n
cordf$n <- NULL



#assign tissue to column
cordf$train_tissue <- NA
cordf$train_tissue[which(cordf$train %in% c( "Hao_etal","Stephenson_etal_Covid","Stephenson_etal_Healthy" ))] <-'PBMC'
cordf$train_tissue[which(cordf$train %in% c( "Leader_etal"))] <-'Lung'
cordf$train_tissue[which(cordf$train %in% c( "Zhang_etal"))] <-'Synovium'
cordf$test_tissue <- NA
cordf$test_tissue[which(cordf$test %in% c( "Hao_etal","Stephenson_etal_Covid","Stephenson_etal_Healthy" ))] <-'PBMC'
cordf$test_tissue[which(cordf$test %in% c( "Leader_etal"))] <-'Lung'
cordf$test_tissue[which(cordf$test %in% c( "Zhang_etal"))] <-'Synovium'


#write to file
write.csv(cordf, file =paste0(outdir,'/dataset_to_dataset.csv'))
...
#handle the stephenson healthy vs stephenson covid cases - we want to drop these because they are within dataset and tissue but between disease 
ind1 <- which(cor_longdf$test == 'Stephenson_etal_Covid' & cor_longdf$train == 'Stephenson_etal_Healthy')
ind2 <- which(cor_longdf$test == 'Stephenson_etal_Healthy' & cor_longdf$train == 'Stephenson_etal_Covid')

subdf <- cor_longdf[-c(ind1,ind2),]


subdf$cat <- factor(subdf$cat, levels = c('within dataset', 'between dataset, within tissue', 'between dataset, between tissue' ))

subdf$method <- factor(subdf$method, levels = names(method_cols))
p <- ggplot(subdf, aes(x = cat, y = value))+
...
#write and save the mean with dataset and between dataset prediction corr
subdf$cat <-as.character(subdf$cat)
subdf$cat[which(subdf$cat != 'within dataset')] <- 'between_dataset'
agg <- subdf %>% group_by(cat, method) %>% dplyr::summarize(meanval = mean(value))

write.csv(agg, file = paste0(outdir,'/within_between_mean_predcor.csv'))

within <- agg[agg$cat == 'within dataset',]
within <- within[within$method != 'scTranslator_pretrain',]
within$rankscore <- rank(within$meanval) / dim(within)[1]

between <- agg[agg$cat != 'within dataset',]
between <- between[between$method != 'scTranslator_pretrain',]
between$rankscore <- rank(between$meanval) / dim(between)[1]
...
ggsave(p, file = paste0(outdir,'overall_Seurat_prediction_violin_Hao_vs_Zhang.png'), width=6, height=3)
...
p <- ggplot(df, aes(x = feat, y =reorder(y.label, desc(y.label)), fill = value))+
  ...
  theme_classic()+
  scale_fill_gradientn(colors = geyser_adjust %>% rev(), limits = c(-1,1), name = 'pred. corr.', na.value  = 'light grey')+
  theme(axis.text.x = element_text(angle=90, hjust=1, vjust=0.5),
...
ggsave(p, file = paste0(outdir,'usability_table.png'), width=12, height=7)
...
## SUPP FIG 5A Benchmarking performance over ascending fractions
...
for(i in names(predset)){
  ...
  agg <- NRMSEdf %>% group_by(method) %>% dplyr::summarise(meanval = mean(na.omit(value)), varval = var(na.omit(value)), medval = median(na.omit(value)))
  agg$partition <- i
  NRMSESet[[as.character(i)]] <- agg
}
...
ggsave(p,  file=paste0(outdir,'/benchmarking_lineplot.png'), width=12, height=3.5)
```
...
```{R, RUNTIME METRICS}
duration <- read.csv(paste0(savedir,'/benchmarking/benchmarking_runtime_min.csv'), row.names = 1)
...
keep <- c("scMMT" , "SPECK" ,"BABEL", "sciPENN" , "cTPnet" , "totalVI" ,"scLinear" , "Seurat","scTranslator_fewshot", "scTranslator_pretrain")

dataSets <- list(duration = duration,  raw_cpu=raw_cpu,  raw_ram=raw_ram)
...
data$cat <- factor(data$cat, levels= c('total RAM', 'total cpu','runtime (minutes)'  ))

#remove duplicated rows
data <- data[!duplicated(data),]
p <- ggplot(data[which(data$cat %in% c('total RAM','total cpu','runtime (minutes)'  )), ], aes(x=name, y=value, fill = name, color=name))+
...
ggsave(p,  file=paste0(outdir,'/runtime_cpu_RAM.png'), width=12, height=4)
...
## SUPP FIG 7 Dataset composition
