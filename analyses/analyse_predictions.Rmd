...
for(n in names(benchmark_predset[['1']][['pred']])){
  feat <- ref$protein
  ind <- rownames( benchmark_predset[['1']][['pred']][[n]]) %in% feat
  pred <- benchmark_predset[['1']][['pred']][[n]][ind,, drop=F]
  ...
  for(ct in categories){
    ...
    keepfeat <- intersect(names(cvals1), names(cvals2))
    df <- data.frame(pred_prot=cvals1[keepfeat],rna_prot=cvals2[keepfeat], celltype=ct, method = n ,medexpr = medvals[keepfeat], feature = [keepfeat)
...
```{r}
#fetch files
file <- list.files(paste0(savedir,'/benchmarking/method_predictions') ,pattern = 'feature_importance', full.names = T)
imp <- read.csv(file, header = F)

#relabel
testing_prot <- read.csv(paste0(savedir,'/benchmarking/testing_files/1_1_testing_data_prot_raw.csv'))
testing_rna <- read.csv(paste0(savedir,'/benchmarking/testing_files/1_1_testing_data_rna_raw.csv'))

rownames(imp) <- testing_prot[,1]
colnames(imp) <- testing_rna[,1]
